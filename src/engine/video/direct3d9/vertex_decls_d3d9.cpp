...
bool VertexDeclsD3D9::initialize() {
	for (uint i = 0; i < COUNT; ++i)
		CHECKED_D3D_CALL(Engine::inst().videoImplD3D9->device().CreateVertexDeclaration(ELEMENTS[i], &vertexDecls_[i]));
...
void VertexDeclsD3D9::activate(const Type type) {
	...
	CHECKED_D3D_CALL_A(Engine::inst().videoImplD3D9->device().SetVertexDeclaration(vertexDecls_[type]));
}
