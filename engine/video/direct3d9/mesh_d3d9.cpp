...
: vertexBuffer_(NULL), indexBuffer_(NULL), 
  vertexCount_(0), verticesCapacity_(0),
  indexCount_(0), indicesCapacity_(0),
  effect_(effect) {
	assert(effect_);
}
...
void MeshD3D9::setBuffers(const void* const vertices, const unsigned vertexCount, const unsigned vertexSize,
						  const short* const indices, const unsigned indexCount) {
	assert(vertices && indices && vertexCount && indexCount);

	vertexCount_ = vertexCount;
	vertexSize_ = vertexSize;
	const unsigned verticesSize = vertexCount * vertexSize;
	...
	const unsigned indicesSize = indexCount * sizeof(short);
